...
for tup in x_tups_train:
	all_classes.append(tup[-1])

class_freq = collections.Counter(all_classes)

class_list = list(class_freq.keys())
...
p = []
sum_all = sum(class_freq)

for j in class_freq:
	p.append(float(j)/sum_all)

#devset
predictions_dev = np.random.choice(class_list, size=len(x_tups_dev), p=p)

#testset
predictions_test = np.random.choice(class_list, size=len(x_tups_test), p=p)
